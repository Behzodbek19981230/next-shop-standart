...
interface CartItem {
  id: number;
  img: StaticImageData; // Type for static images imported in Next.js
  product: string;
  quantity: number;
  price: number;
}

const MyCartDropdown: React.FC = () => {
  ...
  const [cartItem, setCartItem] = useState<number>(cartData.length);
  ...
  const removeItem = (id: number) => {
    ...
    setCartItem(updatedCartData.length);
    // Update cart items and total logic
  };
          ...
          <SimpleBar style={{ maxHeight: "300px" }}>
            <div className="p-2">
              <div
                ...
                id="empty-cart"
                style={{ display: cartItem === 0 ? "block" : "none" }}
              >
